...
if (has_v2ray or has_xray) and #nodes_table > 0 then
    ...
    for k, v in pairs(shunt_list) do
        ...
        local id = "main_node"
        o = s:taboption("Main", ListValue, v.id .. "." .. id, string.format('* <a style="color:red">%s</a>', translate("Default Preproxy")))
        o:depends("tcp_node", v.id)
        o:value("nil", translate("Close"))
        for k1, v1 in pairs(normal_list) do
            o:value(v1.id, v1["remark"])
...
if has_chnlist and api.is_finded("chinadns-ng") then
    o = s:taboption("DNS", Flag, "chinadns_ng", translate("ChinaDNS-NG"))
    o.default = "0"
    if api.is_finded("smartdns") then
...
tips.cfgvalue = function(t, n)
    return string.format('<a style="color: red" href="%s">%s</a>', api.url("acl"), translate(""))
end
...
loglevel = s:taboption("log", ListValue, "loglevel", "v2ray " .. translate("Log Level"))
...
if has_v2ray or has_xray then
    o = s:option(Value, "http_port", "HTTP " .. translate("Listen Port") .. " " .. translate(""))
    o.default = 0
    o.datatype = "port"
end
