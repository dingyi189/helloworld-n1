...
-- 负载均衡列表
balancing_node = s:option(DynamicList, "balancing_node", translate("Load balancing node list"))
...
default_node = s:option(ListValue, "default_node", string.format(translate("Default")))
...
if #nodes_table > 0 then
    o = s:option(ListValue, "main_node", string.format(translate("Default Preproxy")))
    o:value("nil", translate("Close"))
    for k, v in pairs(nodes_table) do
        o:value(v.id, v.remarks)
...
uuid = s:option(Value, "uuid", translate("UUID"))
uuid.password = true
...
tls_allowInsecure = s:option(Flag, "tls_allowInsecure", translate("allowInsecure"))
...
ws_earlyDataHeaderName = s:option(Value, "ws_earlyDataHeaderName", translate("Early data header name"))
...
quic_key = s:option(Value, "quic_key", translate("Key"))
...
-- [[ gRPC部分 ]]--
grpc_serviceName = s:option(Value, "grpc_serviceName", translate("ServiceName"))
grpc_serviceName:depends("transport", "grpc")

grpc_mode = s:option(ListValue, "grpc_mode", translate("Transfer mode"))
grpc_mode:value("gun")
